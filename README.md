...
1. [Install Poetry](https://python-poetry.org/docs/#installation)
   ...
   Once Poetry is installed you can execute the following command to make sure it is setup successfully: 
   
   `poetry --version`
   
   If you see something like Poetry (version 1.2.0) then you are ready to use Poetry.

   In case Poetry doesn't work after installation on Ubuntu set the path to Poetry environment manually using:
   
   `export PATH=$HOME/.poetry/bin:$PATH`

2. Activate the virtual environment (it was auto-generated by Poetry in the previous step):
   
   `poetry shell`

3. Make sure you create `.env` using `.env.template` and update the values of corresponding environment variables
...
Tip: if you're using GNU/Linux or MacOS: `printenv` to print the environment variables and check all configurations.
...
- `poetry --version` - to check if Poetry is installed successfully 
