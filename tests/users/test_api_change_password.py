...
from app.messages import PASSWORD_SUCCESSFULLY_UPDATED, USER_INPUTS_SPACE_IN_PASSWORD, AUTHORISATION_TOKEN_IS_MISSING, \
    TOKEN_HAS_EXPIRED
...
class TestUserChangePasswordApi(BaseTestCase):
    ...
    def test_change_password_to_correct_new_one(self):
        with self.client:
            ...
            response = self.client.put('/user/change_password',
                                       json={'current_password': self.current_password,
    ...
    def test_change_password_to_empty_one(self):
        with self.client:
            new_password = ''
            expected_response = {"message": get_length_validation_error_message("new_password", PASSWORD_MIN_LENGTH,
                                                                                PASSWORD_MAX_LENGTH)}
            response = self.client.put('/user/change_password',
                                       json={'current_password': self.current_password,
                                             'new_password': new_password},
    ...
    def test_change_password_with_authentication_token_expired(self):
        with self.client:
            new_password = '123new_password'
            auth_header = get_test_request_header(self.first_user.id, datetime.timedelta(seconds=-1))
