...
class TestAcceptMentorshipRequestApi(MentorshipRelationBaseTestCase):
    ...
    def test_recepient_already_in_relation_accept_request(self):
        ...
        db.session.commit()
        self.assertEqual(MentorshipRelationState.ACCEPTED, mentorship_relation_current.state) # current
        self.assertEqual(MentorshipRelationState.PENDING, self.mentorship_relation.state) # new
        with self.client:
            response = self.client.put('/mentorship_relation/%s/accept' % self.mentorship_relation.id,
                                       headers=get_test_request_header(self.second_user.id))
