...
class TestMentorshipRelationListingselfDAO(MentorshipRelationBaseTestCase):

    # Setup consists of adding 2 users into the database
    # User 1 is the mentorship relation requester = action user
    # User 2 is the receiver
    def setUp(self):
        super(TestMentorshipRelationListingselfDAO, self).setUp()

    def test_dao_reject_non_existing_mentorship_request(self):
        result = self.dao.reject_request(self.first_user.id, 123)
    ...
    def test_dao_sender_does_not_exist_mentorship_request(self):
        result = self.dao.reject_request(123, self.mentorship_relation.id)
    ...
    def test_dao_requester_tries_to_reject_mentorship_request(self):
        result = self.dao.reject_request(self.first_user.id,
                                         self.mentorship_relation.id)

        self.assertEqual((messages.USER_CANT_REJECT_REQUEST_SENT_BY_USER, 400),
                         result)
        self.assertEqual(MentorshipRelationState.PENDING,
                         self.mentorship_relation.state)

    def test_dao_receiver_rejects_mentorship_request(self):
        result = self.dao.reject_request(self.second_user.id,
                                         self.mentorship_relation.id)

        self.assertEqual(
            (messages.MENTORSHIP_RELATION_WAS_REJECTED_SUCCESSFULLY, 200),
            result)
