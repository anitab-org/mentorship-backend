...
class TestMentorshipRelationListingDAO(MentorshipRelationBaseTestCase):
    ...
    def test_dao_cancel_non_existing_mentorship_request(self):
        result = self.dao.cancel_relation(self.first_user.id, 123)
    ...
    def test_dao_sender_does_not_exist_mentorship_request(self):
        result = self.dao.cancel_relation(123, self.mentorship_relation.id)
    ...
    def test_dao_receiver_cancel_mentorship_request(self):
        ...
        db.session.commit()
        result = self.dao.cancel_relation(self.second_user.id,
                                          self.mentorship_relation.id)

        self.assertEqual(
            (messages.MENTORSHIP_RELATION_WAS_CANCELLED_SUCCESSFULLY, 200),
            result)
    ...
    def test_dao_sender_cancel_mentorship_request(self):
        ...
        result = self.dao.cancel_relation(self.first_user.id,
                                          self.mentorship_relation.id)
