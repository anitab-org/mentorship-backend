...
class TestMentorshipRelationCreationDAO(MentorshipRelationBaseTestCase):
    ...
    def test_dao_create_mentorship_relation_with_good_args_mentor_is_sender(
            self):
        # Use DAO to create a mentorship relation

        result = self.dao.create_mentorship_relation(self.first_user.id,
                                                     self.data)
        ...
        self.assertEqual(self.end_date_example.timestamp(),
        ...
        self.assertEqual(2, len(self.first_user.mentor_relations))
        ...
        self.assertEqual(2, len(self.second_user.mentee_relations))
        self.assertEqual(query_mentorship_relation,
    ...
    def test_dao_create_mentorship_relation_with_good_args_mentor_is_receiver(
        ...
        result = self.dao.create_mentorship_relation(self.second_user.id,
                                                     self.data)

        self.assertEqual(messages.MENTORSHIP_RELATION_WAS_SENT_SUCCESSFULLY,
                         result[0])

        query_mentorship_relation = MentorshipRelationModel.query[1]

        self.assertTrue(query_mentorship_relation is not None)

        self.assertEqual(2, query_mentorship_relation.id)
        ...
        self.assertEqual(2, len(self.first_user.mentor_relations))
        self.assertEqual(query_mentorship_relation,
                         self.first_user.mentor_relations[1])
        self.assertEqual([], self.first_user.mentee_relations)

        # assert mentees' mentor_relations and mentee_relations
        self.assertEqual(2, len(self.second_user.mentee_relations))
        self.assertEqual(query_mentorship_relation,
                         self.second_user.mentee_relations[1])
        self.assertEqual([], self.second_user.mentor_relations)
    ...
    def test_dao_create_mentorship_relation_with_non_existent_mentor(self):
        ...
        result = self.dao.create_mentorship_relation(1234, data)

        self.assertDictEqual(messages.MENTOR_DOES_NOT_EXIST, result[0])

        query_mentorship_relation = MentorshipRelationModel.query.all()

        self.assertEqual(1, len(query_mentorship_relation))
    ...
    def test_dao_create_mentorship_relation_with_non_existent_mentee(self):
        ...
        result = self.dao.create_mentorship_relation(self.first_user.id, data)

        self.assertDictEqual(messages.MENTEE_DOES_NOT_EXIST, result[0])

        self.assertEqual(0, len(self.first_user.mentee_relations))

    def test_dao_create_mentorship_relation_with_mentee_already_in_relation(
            self):
    ...
    def test_dao_create_mentorship_relation_with_mentor_already_in_relation(
        ...
        result = self.dao.create_mentorship_relation(self.second_user.id, data)
