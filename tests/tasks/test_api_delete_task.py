...
class TestDeleteTaskApi(TasksBaseTestCase):
    ...
    def test_user_not_in_this_relation(self):
        expected_response = messages.USER_NOT_INVOLVED_IN_THIS_MENTOR_RELATION

        existent_task = self.tasks_list_1.find_task_by_id(1)
        self.assertIsNotNone(existent_task)
        ...
        self.assertEqual(401, actual_response.status_code)
        task_tried_to_be_deleted = self.tasks_list_1.find_task_by_id(1)
        self.assertIsNotNone(task_tried_to_be_deleted)
