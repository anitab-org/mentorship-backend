...
class TestTestingConfig(TestCase):
    def create_app(self):
        ...
        application.config[
            'SECRET_KEY'] = secret_key if secret_key else 'TEST_SECRET_KEY'
        return application
    ...
    def test_app_testing_config(self):
        ...
        self.assertEqual(timedelta(weeks=1),
                         application.config['JWT_ACCESS_TOKEN_EXPIRES'])
        self.assertEqual(timedelta(weeks=4),
...
class TestDevelopmentConfig(TestCase):
    ...
    def test_app_development_config(self):
        ...
        self.assertFalse(application.config['SQLALCHEMY_TRACK_MODIFICATIONS'])
        self.assertEqual('sqlite:///dev_data.db',
                         application.config['SQLALCHEMY_DATABASE_URI'])
        self.assertIsNotNone(current_app)
...
class TestProductionConfig(TestCase):
    ...
    def test_app_production_config(self):
        ...
        self.assertFalse(application.config['TESTING'])
        self.assertFalse(application.config['SQLALCHEMY_TRACK_MODIFICATIONS'])
        self.assertEqual('sqlite:///prod_data.db',
                         application.config['SQLALCHEMY_DATABASE_URI'])
        self.assertIsNotNone(current_app)
