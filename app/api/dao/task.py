...
class TaskDAO:
    ...
    def create_task(user_id, mentorship_relation_id, data):
        ...
        Returns:
            ...
            containing a key 'message' indicating in its value if the task
            creation was succesful or not as a string. The last element is
            the HTTPresponse code.
        """

        description = data['description']

        user = UserModel.find_by_id(user_id)
        rel = MentorshipRelationModel.find_by_id(
    ...
    def list_tasks(user_id, mentorship_relation_id):
        ...
        Returns:
            ...
            relation. otherwise, it returns a two element list where the first
            element is a dictionary containing a key 'message' indicating in
            its value if there were any problem finding user's tasks in the
            specified mentorship relation as a string. The last element is the
            HTTP response code
        """

        user = UserModel.find_by_id(user_id)
        rel = MentorshipRelationModel.find_by_id(mentorship_relation_id)
        if rel is None:
            return messages.MENTORSHIP_RELATION_DOES_NOT_EXIST, 404

        if not (user_id == rel.mentee_id or user_id == rel.mentor_id):
            return messages.USER_NOT_INVOLVED_IN_THIS_MENTOR_RELATION, 401

        all_tasks = rel.tasks_list.tasks
    ...
    def delete_task(user_id, mentorship_relation_id, task_id):
        ...
        Returns:
            A two element list where the first element is a dictionary
            containing a key 'message' indicating in its value if the task was
            deleted succesfully or not as a string. The last element is the
            HTTP response code.
        """
        ...
        task = rel.tasks_list.find_task_by_id(task_id)
        if task is None:
        ...
        rel.tasks_list.delete_task(task_id)
    ...
    def complete_task(user_id, mentorship_relation_id, task_id):
        ...
        Returns:
            A two element list where the first element is a dictionary
            containing a key 'message' indicating in its value if the task was
            set to complete succesfully or not as a string. The last element is
