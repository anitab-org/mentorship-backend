    ...
    if len(password_check) < 8:
        ...
        for ch in password_check:
            if ch.isupper():
                ...
                break
        if flag!=1:
            return messages.ATLEAST_ONE_UPPERCASE,400
        for ch in password_check:
            if ch.isdigit():
                flag=2
                break
        if flag!=2:
            return messages.ATLEAST_NUMBER,400
        regex=re.compile('[{(~!@#$%^&*_-|\:;?/+=)}]')
        if(regex.search(password_check) is None):
            return messages.ATLEAST_ONE_SPECIAL_CHARACTER,400
        else:
            flag=3
    if flag==3:
        return True
