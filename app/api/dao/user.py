...
class UserDAO:
    ...
    def create_user(data):
        ...
        if existing_user:
            return {"message": USER_USES_A_USERNAME_THAT_ALREADY_EXISTS }, 400
        else:
            existing_user = UserModel.find_by_email(data['email'])
            if existing_user:
                return {"message": USER_USES_AN_EMAIL_ID_THAT_ALREADY_EXISTS }, 400
        ...
        return {"message": USER_WAS_CREATED_SUCCESSFULLY }, 200
    ...
    def delete_user(user_id):
        ...
        if user.is_admin:
            ...
            if admins_list_count <= UserDAO.MIN_NUMBER_OF_ADMINS:
                return {"message": USER_CANNOT_DELETE_HIS_ACCOUNT_WHILE_HE_IS_THE_ONLY_ADMIN_LEFT }, 400

        if user:
            user.delete_from_db()
            return {"message": USER_SUCCESSFULLY_DELETED }, 200
    ...
    def update_user_profile(user_id, data):
        ...
        return {"message": USER_SUCCESSFULLY_UPDATED }, 200
    ...
    def change_password(user_id, data):
        ...
        if user.check_password(current_password):
            ...
            user.save_to_db()
            return {"message": PASSWORD_SUCCESSFULLY_UPDATED }, 201

        return {"message": USER_ENTERED_INCORRECT_PASSWORD }, 400
    ...
    def confirm_registration(token):
        ...
        if email_from_token is False or email_from_token is None:
            return {"message": EMAIL_EXPIRED_OR_TOKEN_IS_INVALID }, 400

        user = UserModel.find_by_email(email_from_token)
        if user.is_email_verified:
            return {"message": ACCOUNT_ALREADY_CONFIRMED }, 200
        else:
            user.is_email_verified = True
            user.email_verification_date = datetime.now()
            user.save_to_db()
            return {"message": ACCOUNT_ALREADY_CONFIRMED_AND THANKS }, 200
