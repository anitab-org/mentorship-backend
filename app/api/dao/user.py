...
class UserDAO:
    ...
    def list_users(user_id, is_verified=None):
        ...
        # If 'is_verified' is True, users_list is filtered with the verified users, else users_list remains same.
        if is_verified:
            users_list = list(filter(lambda user: user.is_email_verified, users_list))

        list_of_users = list(map(lambda user: user.json(), users_list))
