...
class MentorshipRelationDAO:
    ...
    def accept_request(user_id, request_id):
        ...
        if request is None:
            return {"message": MENTORSHIP_RELATION_REQUEST_DOES_NOT_EXIST }, 404

        # verify if request is in pending state
        if request.state is not MentorshipRelationState.PENDING:
            return {"message": MENTOR_RELATIONSHIP_IS_NOT_IN_PENDING_STATE}, 400

        # verify if I'm the receiver of the request
        if request.action_user_id is user_id:
            return {"message": USER_CANNOT_ACCEPT_A_MENTORSHIP_REQUEST_SENT_BY_HIMSELF}, 400

        # verify if I'm involved in this relation
        if not (request.mentee_id is user_id or request.mentor_id is user_id):
            return {"message": USER_CANNOT_ACCEPT_A_MENTORSHIP_RELATION_WHERE_HE_IS_NOT_INVOLVED}, 400
        ...
        for request in requests:
            if request.state is MentorshipRelationState.ACCEPTED:
                return {"message": USER_IS_INVOLVED_IN_A_MENTORSHIP_RELATION }, 400

        # All was checked
        request.state = MentorshipRelationState.ACCEPTED
        request.save_to_db()

        return {"message": MENTORSHIP_RELATION_WAS_ACCEPTED_SUCCESSFULLY }, 200
    ...
    def reject_request(user_id, request_id):
        ...
        # verify if I'm the receiver of the request
        if request.action_user_id is user_id:
            return {"message": USER_CANNOT_REJECT_A_RELATIONSHIP_REQUEST_SENT_BY_HIMSELF_FOR_A_MENTOR }, 400

        # verify if I'm involved in this relation
        if not (request.mentee_id is user_id or request.mentor_id is user_id):
            return {"message": USER_CANNOT_REJECT_A_RELATIONSHIP_REQUEST_WHERE_HE_IS_NOT_INVOLVED }, 400

        # All was checked
        request.state = MentorshipRelationState.REJECTED
        request.save_to_db()

        return {"message": MENTORSHIP_RELATION_WAS_REJECTED_SUCCESSFULLY }, 200
    ...
    def cancel_relation(user_id, relation_id):
        ...
        if request.state is not MentorshipRelationState.ACCEPTED:
            return {"message": MENTOR_RELATIONSHIP_STATUS_IS_NOT_IN_ACCEPTED_STATE }, 400

        # verify if I'm involved in this relation
        if not (request.mentee_id is user_id or request.mentor_id is user_id):
            return {"message": USER_CANNOT_CANCEL_A_RELATIONSHIP_REQUEST_WHERE_HE_IS_NOT_INVOLVED }, 400
    ...
    def delete_request(user_id, request_id):
        ...
        if request.action_user_id is not user_id:
            return {"message": USER_CANNOT_DELETE_A_RELATIONSHIP_REQUEST_WHICH_HE_DID_NOT_CREATE }, 400

        # All was checked
        request.delete_from_db()

        return {"message": MENTORSHIP_RELATION_WAS_DELETED_SUCCESSFULLY }, 200
    ...
    def list_current_mentorship_relation(user_id):
        ...
        return {"message": USER_IS_NOT_IN_A_MENTORED_RELATIONSHIP_CURRENTLY }, 200
