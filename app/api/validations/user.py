...
def validate_user_registration_request_data(data):
    ...
    if 'name' not in data:
        return {"message": NAME_FIELD_IS_MISSING }
    if 'username' not in data:
        return {"message": USERNAME_FIELD_IS_MISSING }
    if 'password' not in data:
        return {"message": PASSWORD_FIELD_IS_MISSING }
    if 'email' not in data:
        return {"message": EMAIL_FIELD_IS_MISSING }
    if 'terms_and_conditions_checked' not in data:
        return {"message": TERMS_AND_CONDITIONS_FIELD_IS_MISSING }
    ...
    if not (isinstance(name, str) and isinstance(username, str) and isinstance(password, str)):
        return {"message": NAME_USERNAME_AND_PASSWORD_ARE_NOT_IN_STRING_FORMAT }
    ...
    if terms_and_conditions_checked is False:
        return {"message": TERMS_AND_CONDITIONS_ARE_NOT_CHECKED }

    if not is_name_valid(name):
        return {"message": NAME_INPUT_BY_USER_IS_INVALID }

    if not is_email_valid(email):
        return {"message": EMAIL_INPUT_BY_USER_IS_INVALID }

    if not is_username_valid(username):
        return {"message": USERNAME_INPUT_BY_USER_IS_INVALID }
...
def validate_update_profile_request_data(data):
    ...
    if not data:
        return {"message": NO_DATA_FOR UPDATING_PROFILE_WAS_SENT }
    ...
    if username:
        ...
        if not is_username_valid(username):
            return {"message": NEW_USERNAME_INPUT_BY_USER_IS_INVALID }
    ...
    if 'need_mentoring' in data and data['need_mentoring'] is None:
        return {"message": FIELD_NEED_MENTORING_IS_NOT_VALID }

    if 'available_to_mentor' in data and data['available_to_mentor'] is None:
        return {"message": FIELD_AVAILABLE_TO_MENTOR_IS_NOT_VALID }
...
def validate_new_password(data):
    if 'current_password' not in data:
        return {"message": CURRENT_PASSWORD_FIELD_IS_MISSING }
    if 'new_password' not in data:
        return {"message": NEW_PASSWORD_FIELD_IS_MISSING }

    new_password = data['new_password']

    if " " in new_password:
        return {"message": USER_INPUTS_SPACE_IN_PASSWORD }
