...
from app.utils.validation_utils import is_name_valid, is_email_valid, \
    is_username_valid, validate_length, \
    get_stripped_string
...
def validate_user_registration_request_data(data):
    ...
    if not (isinstance(name, str) and isinstance(username, str) and isinstance(
            password, str)):
    ...
    is_valid = validate_length(len(get_stripped_string(username)),
                               USERNAME_MIN_LENGTH, USERNAME_MAX_LENGTH,
                               'username')
    if not is_valid[0]:
        return is_valid[1]

    is_valid = validate_length(len(get_stripped_string(password)),
...
def validate_update_profile_request_data(data):
    ...
    if organization:
        is_valid = validate_length(len(get_stripped_string(organization)), 0,
                                   ORGANIZATION_MAX_LENGTH, 'organization')
        if not is_valid[0]:
            return is_valid[1]

    slack_username = data.get('slack_username', None)
    if slack_username:
        is_valid = validate_length(len(get_stripped_string(slack_username)), 0,
    ...
    if social_media_links:
        is_valid = validate_length(
            len(get_stripped_string(social_media_links)), 0,
            SOCIALS_MAX_LENGTH,
            'social_media_links')
        if not is_valid[0]:
            return is_valid[1]
...
def validate_new_password(data):
    ...
    is_valid = validate_length(len(get_stripped_string(new_password)),
                               PASSWORD_MIN_LENGTH, PASSWORD_MAX_LENGTH,
                               'new_password')
    if not is_valid[0]:
        return is_valid[1]
