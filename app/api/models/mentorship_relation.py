...
def add_models_to_namespace(api_namespace):
    api_namespace.models[send_mentorship_request_body.name] = (
        send_mentorship_request_body)
    api_namespace.models[mentorship_request_response_body.name] = (
        mentorship_request_response_body)
    api_namespace.models[relation_user_response_body.name] = (
        relation_user_response_body)
    api_namespace.models[create_task_request_body.name] = (
        create_task_request_body)
    api_namespace.models[list_tasks_response_body.name] = (
...
send_mentorship_request_body = Model(
    'Send mentorship relation request model',
    {
        ...
        'end_date': fields.Float(
            required=True,
            description='Mentorship relation end date in UNIX' +
            ' timestamp format'),
        'notes': fields.String(
            required=True,
...
mentorship_request_response_body = Model(
    'List mentorship relation request model', {
        ...
        'creation_date': fields.Float(
            required=True,
            description='Mentorship relation creation date in UNIX timestamp' +
            ' format'),
        'accept_date': fields.Float(
            required=True,
            description='Mentorship relation acceptance date in UNIX' +
            ' timestamp format'),
        'start_date': fields.Float(
            required=True,
            description='Mentorship relation start date in UNIX' +
            ' timestamp format'),
        'end_date': fields.Float(
            required=True,
            description='Mentorship relation end date in UNIX timestamp' +
            ' format'),
